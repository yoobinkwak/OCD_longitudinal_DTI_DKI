...
class striatum:
    def __init__(self, param):
        ...
        for side_s in 'lh', 'rh':
            ...
            segDir = os.path.join(subjDir, 'Segmentation_YKwak', side)
            ...
            ## Save the volume of the striatum (in subject space) in self.striatum_vol
            striatum_file = os.path.join(roiDir, '{0}_striatum.nii.gz'.format(side_s))
            striatum_map = get_map(striatum_file)
            striatum_vol = count_nonzero(striatum_map)

            for cortex in cortices:
                seeds_in_raw = os.path.join(segDir, 'seeds_to_{side_s}_{cortex}_GMmasked_2mm.nii.gz'.format(side_s=side_s, cortex=cortex))
                ...
                for threshold in ['5', '10', '20', '90', '95']:
                    ...
                    seeds_in_dti = os.path.join(thrDir, '{threshold}_seeds_to_{side_s}_{cortex}_GMmasked_2mm.nii.gz'.format(side_s=side_s, cortex=cortex, threshold=threshold))
                    ...
                    seeds_in_dki = os.path.join(thrDir, 'dki_{threshold}_seeds_to_{side_s}_{cortex}_GMmasked_2mm.nii.gz'.format(side_s=side_s, cortex=cortex, threshold=threshold))
                    ...
                    df = pd.concat([df,
                                    pd.DataFrame([[subject, side, cortex, threshold,
                                                 ...
                                                 columns = ['subject',
                                                            ...
                                                            'striatum_seg_mk',
                                                            'dki_total_connectivity',
                                                            'dki_striatum_seg_volume', 
                                                            'total_connectivity_raw']),
                               ])
        self.df = df.reset_index().drop('index', axis=1)


f = striatum((dataLoc, subjects[0]))

a = [[dataLoc], subjects]
all_combinations = list(itertools.product(*a))
pool = Pool()
outs = pool.map(striatum, all_combinations)
merged_df = pd.concat([x.df for x in outs])
merged_df.to_csv('/Volumes/CCNC_4T/OCD_longitudinal_20180611/analyses/BL/BL_striatum.csv')
